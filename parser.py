...
# Раскрываем символ '~' в абсолютный путь
parsed_file_path = os.path.expanduser(parsed_file_path)
async def send_document_async():
    # Открытие файла в бинарном режиме ('rb')
    with open(parsed_file_path, 'rb') as parsed_file:
        # Создание объекта бота
        bot = Bot(token=BOT_TOKEN)
        # Отправка файла в Телеграм
        await bot.send_document(chat_id=CHAT_ID, document=parsed_file)

# Запуск асинхронной функции в цикле событий asyncio
loop = asyncio.get_event_loop()
loop.run_until_complete(send_document_async())
...
if __name__ == "__main__":
    ParseWB("https://www.wildberries.ru/catalog/180954180/detail.aspx").parse()
